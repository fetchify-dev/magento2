...
namespace Fetchify\Fetchify\Block\Adminhtml\Config;
		...
		<script>
			...
			function handleTokens(tokens) {
				if (token_option == 'single') {
					document.querySelector('#fetchify_main_main_options_accesstoken').value = tokens.single;
				} else if (token_option == 'dual') {
					document.querySelector('fetchify_main_main_options_accesstoken').value = tokens.internal;
			...
			function get_cc_token() {
				...
				cc_obj.make_popup = function() {
					...
					<div id='crafty_content'>
						...
						<iframe id='crafty_iframe' src='https://account.fetchify.com/#/external/fetch_my_tokens/` + token_option + `' frameborder='0'></iframe>
					</div>`;
